...
#target_taxa = "18S"; forward_primer=c("TTGTACACACCGCCC"); reverse_primer=c("CCTTCYGCAGGTTCACCTAC"); tL = c(130,130);minLen = 50 # 18S V9 (expected length ~ 200bp)
target_taxa = "18S"; forward_primer=c("TTTGTCTGSTTAATTSCG"); reverse_primer=c("CACAGACCTGTTATTGC"); tL = c(130,115);minLen = 50 # 18S V7 (expected length ~ 200bp)
...
taxo_db_sp_16S = "/mnt/HDD/reference_db/silva/silva_nr99_v138.2_toSpecies_trainset.fa.gz"
...
# COI 
classifier = "/srv/users/Oli/COI_insect_classifier.rds"
...
fas_Fs_cut_ne=fas_Fs_cut[which(out_11$demultiplexed>0)]
fas_Rs_cut_ne=fas_Rs_cut[which(out_11$demultiplexed>0)]
...
p <- ggplot(df, aes(x=Var1)) +
 geom_density()+
  ...
  scale_x_continuous(name ="Sequence length (bp)",breaks = c(0,50,100,150,200,250,300,350,374,400,450,500,550), labels = c(0,50100,150,"200","250","300","350","374","400","450","500","550"))
